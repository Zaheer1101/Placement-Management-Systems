...
class StudentRegistrationForm(forms.ModelForm):
    ...
    cgpa = forms.FloatField(required=False, min_value=1, max_value=10.0,
                          widget=forms.NumberInput(attrs={'placeholder': 'e.g., 8.75', 'step': '0.01'}))
    ...
    def save(self, commit=True):
        ...
        if commit:
            user.save()
            # Create student profile with all the fields
            StudentProfile.objects.create(
                user=user,
                plain_password=password,  # Save the exact password
...
class StudentProfileForm(forms.ModelForm):
    ...
    plain_password = forms.CharField(
        required=False,
        label="Password",
        widget=forms.TextInput(attrs={'placeholder': 'Enter new password (leave blank to keep unchanged)'})
    )
    class Meta:
        model = StudentProfile
        exclude = ['user']
        widgets = {
            'is_placed': forms.Select(choices=[(True, 'Placed'), (False, 'Not Placed')]),
