...
def mark_all_notifications_read(request):
    # TODO: Replace with actual notification model logic
    # Example: Notification.objects.filter(user=request.user, read=False).update(read=True)
...
def clear_all_notifications(request):
    # TODO: Replace with actual notification model logic
    # Example: Notification.objects.filter(user=request.user).delete()
    return JsonResponse({'success': True})

@login_required
@company_required
def all_applicants_view(request):
    jobs = JobPosting.objects.filter(company=request.user.company_profile)
    applications = Application.objects.filter(job__in=jobs).select_related('student__user', 'job')
    context = {
        'applications': applications
    }
    return render(request, 'company/all_applicants.html', context)

@login_required
@company_required
def shortlisted_candidates_view(request, job_id):
    job = get_object_or_404(JobPosting, id=job_id, company=request.user.company_profile)
    applications = Application.objects.filter(job=job, status='Shortlisted').select_related('student__user')
    context = {
        'applications': applications,
        'job': job
    }
    return render(request, 'company/shortlisted_candidates.html', context)

# AJAX: Update application status (shortlist/reject)
@login_required
@company_required
@require_GET
...
# Admin: Edit Job Status
@login_required
@admin_required
def edit_job_status_view(request, job_id):
    ...
    if request.method == 'POST':
        ...
        if status == 'pending':
            ...
            job.application_deadline = timezone.now() + timezone.timedelta(days=7)  # Example: set deadline to 7 days from now
        elif status == 'closed':
            job.is_approved = True
            job.application_deadline = timezone.now() - timezone.timedelta(days=1)  # Example: set deadline to past
        job.save()
        messages.success(request, 'Job status updated successfully.')
        return redirect('core:manage_jobs')
    return redirect('core:manage_jobs')
# Admin: Edit Company
@login_required
@admin_required
def edit_company_view(request, user_id):
    company_user = get_object_or_404(User, id=user_id, role='company')
    company_profile = get_object_or_404(CompanyProfile, user=company_user)
    if request.method == 'POST':
        user_form = UserUpdateForm(request.POST, instance=company_user)
        profile_form = CompanyProfileForm(request.POST, request.FILES, instance=company_profile)
        ...
        user_form = UserUpdateForm(instance=company_user)
        profile_form = CompanyProfileForm(instance=company_profile)
    return render(request, 'admin/edit_company.html', {
        'user_form': user_form,
...
@login_required
@admin_required
def delete_company_view(request, user_id):
    company_user = get_object_or_404(User, id=user_id, role='company')
    company_profile = get_object_or_404(CompanyProfile, user=company_user)
    if request.method == 'POST':
        company_user.delete()
        ...
        return redirect('core:manage_companies')
    return render(request, 'admin/delete_company_confirm.html', {
        'company': company_profile
    })

# Admin: Export Placement Summary PDF (Demo)
...
def export_summary_pdf_view(request):
    ...
    p.drawString(100, 800, "Placement Summary Report")
    p.setFont("Helvetica", 12)
    p.drawString(100, 770, "This is a demo PDF summary of placement statistics.")
    p.drawString(100, 750, "You can customize this to include real data.")
    p.showPage()
    p.save()
    buffer.seek(0)
...
def edit_student_view(request, user_id):
    ...
    if request.method == 'POST':
        user_form = UserUpdateForm(request.POST, instance=student_user)
        profile_form = StudentProfileForm(request.POST, request.FILES, instance=student_profile)
        if user_form.is_valid() and profile_form.is_valid():
            new_plain_password = profile_form.cleaned_data.get('plain_password', '').strip()
            profile = profile_form.save(commit=False)
            # If password changed, update both User and profile
            if new_plain_password and new_plain_password != student_profile.plain_password:
                profile.plain_password = new_plain_password
                student_user.set_password(new_plain_password)
                student_user.save()
            else:
                profile.plain_password = student_profile.plain_password
                user_form.save()
            profile.save()
            messages.success(request, 'Student profile updated successfully.')
            return redirect('core:manage_students')
    else:
        user_form = UserUpdateForm(instance=student_user)
...
def delete_student_view(request, user_id):
    ...
    if request.method == 'POST':
        ...
        return redirect('core:manage_students')
    return render(request, 'admin/delete_student_confirm.html', {
        'student': student_profile
    })
...
def student_register_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            user = form.save()
            # Save the plain password in the StudentProfile
            student_profile = user.student_profile
            student_profile.plain_password = form.cleaned_data.get('password')
...
def login_view(request):
    # Clear any leftover messages when showing the login page (GET) so
    # stale success/error notifications from previous sessions don't persist.
    if request.method == 'GET':
        list(messages.get_messages(request))

    if request.method == 'POST':
        form = AuthenticationForm(request, data=request.POST)
        # Enforce that the user checked the "Remember me" checkbox
        remember_checked = request.POST.get('remember_me')
        if not remember_checked:
            # Add a non-field error so it shows up with the form errors
            form.add_error(None, 'You must check "Remember me" to log in.')
        else:
            if form.is_valid():
                ...
                if user is not None:
                    login(request, user)
                    # If remember_me checked, set a longer session expiry (2 weeks)
                    try:
...
def logout_view(request):
    # Consume any stored messages to avoid showing them after logout/redirect
    list(messages.get_messages(request))
    logout(request)
...
def job_detail_view(request, job_id):
    ...
    if student_profile:
        ...
        if student_profile.branch in allowed_branches:
            # Add other eligibility checks here (CGPA, backlogs, etc.)
            application_status = "ELIGIBLE"
        # If already applied, override status
...
def apply_for_job_view(request, job_id):
    ...
    student_cgpa = student_profile.cgpa if student_profile.cgpa is not None else 0.0
    student_backlogs = student_profile.backlogs if student_profile.backlogs is not None else 0
    # job.min_cgpa or job.max_backlogs may be None (meaning no restriction)
    min_cgpa_ok = True if job.min_cgpa is None else (student_cgpa >= job.min_cgpa)
    max_backlogs_ok = True if job.max_backlogs is None else (student_backlogs <= job.max_backlogs)
    allowed_branches = [b.strip() for b in job.allowed_branches.split(',')] if job.allowed_branches else []
    branch_ok = (student_profile.branch in allowed_branches) if allowed_branches else True

    is_eligible = min_cgpa_ok and max_backlogs_ok and branch_ok
    ...
    if request.method == 'POST':
        ...
        if app_ids and action:
            ...
            if action == 'shortlist':
                ...
                # For offers we need to mark each application and student individually
                offered_count = 0
                for app in selected_applications.select_related('student__user'):
                    try:
                        app.status = 'Offered'
                        app.save()
                        # Mark the student profile as placed
                        ...
                        continue
                messages.success(request, f'{offered_count} candidate(s) have been offered and marked as placed.')
            elif action == 'applied':
                updated_count = selected_applications.update(status='Applied')
                messages.info(request, f'{updated_count} candidate(s) status set to Applied.')
...
def manage_students_view(request):
    ...
    # Get the plain password for display
    for student in students_list:
        # Show the exact password that was used during registration
        student.display_password = student.plain_password if student.plain_password else "Password not available"
    ...
    if name_query:
        students_list = students_list.filter(
            ...
            Q(user__email__icontains=name_query)
        )
    if branch_query:
        students_list = students_list.filter(branch__icontains=branch_query)
    if status_query:
        if status_query == 'placed':
            students_list = students_list.filter(is_placed=True)
        elif status_query == 'not_placed':
...
@login_required
@admin_required
def admin_add_company_view(request):
    if request.method == 'POST':
        form = CompanyRegistrationForm(request.POST)
        if form.is_valid():
            user = form.save()
